...
class TestBIFReading(TestBase):
    ...
    def assertBayesianNetworkEqual(self, bnA, bnB):
        ...
        for varname in varnamesA:
            ...
            self.assertVariableNodesEqual(varA, varB)


    def assertVariableNodesEqual(self, A, B):
        self.assertEqual(A.name, B.name)
        self.assertListEqual(A.values, B.values)
        self.assertDictEqual(A.properties, B.properties)
        self.assertProbabilityDistributionsOfVariableNodesEqual(A.probdist, B.probdist)


    def assertProbabilityDistributionsOfVariableNodesEqual(self, pA, pB):
        self.assertEqual(len(pA.conditioning_variable_nodes), len(pB.conditioning_variable_nodes))
